...
-- blob You want to keep a description of each actor. You don't think you will be performing queries on a description, so create a column in the table `actor` named `description` and use the data type `BLOB` (Make sure to research the type `BLOB`, as the difference between it and `VARCHAR` are significant).
-- 3a. Add column named description to the table actor. Position it between first_name and last_name.
ALTER TABLE actor
...
-- 4d. The actor `HARPO WILLIAMS` was accidentally entered in the `actor` table as `GROUCHO WILLIAMS`. Write a query to fix the record.
UPDATE actor
...
-- I don't want to delete the current "Address file" here,so i will use address1 instead
...
-- 7g.Write a query to display how much business, in dollars, each store brought in.
